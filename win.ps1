...
# create local user
net user comp064 /add /active:yes /logonpasswordchg:no

# configure users
set-localuser -Name sadm -Password (ConvertTo-SecureString F2CpzhYmSKzB -AsPlainText -Force)
set-localuser -Name comp064 -Password (ConvertTo-SecureString atcWsk68Q4xH -AsPlainText -Force) -PasswordNeverExpires 1 -UserMayChangePassword 0


# set hostname
rename-computer -newname comp064
...
choco install saltminion --params="'/master:sm.fwtg.pp.ua /minion-name:comp064'" -Force -Y
$Text= gc 'C:\salt\conf\minion' -Raw
$Text-replace('\#id\:', 'id: comp064') | Set-Content 'C:\salt\conf\minion'
"comp064" | Set-Content 'C:\salt\conf\minion_id'
...
cd C:\Users\Public\Desktop\Activators\KMSAutoLite
.\KMSAutox64.exe /win=act
.\KMSAutox64.exe /ofs=act
